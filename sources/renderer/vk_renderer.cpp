...
bool VkRenderer::createSwapChain(uint32_t height, uint32_t width)
{
	m_swapchin.createSwapChain(m_device, m_surface);
	m_swapchin.createFrameBuffer(height, width, m_renderpass, m_device.getLogicalDevice());
	return true;
}

bool VkRenderer::createRenderPass(VkSurfaceFormatKHR surface_format, VkDevice logical_device)
{
