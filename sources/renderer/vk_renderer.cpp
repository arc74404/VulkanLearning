...
bool vk_init(VkContext *vk_context, GLFWwindow *window)
{
	...
	// Swap Chain
	{
		...
		VK_CHECK(vkCreateImage(vk_context->device, &image_info, 0, vk_context->swapchain_images));
	...
	// Create Descriptor Set Layouts
	{
		...
		VK_CHECK(vkCreateDescriptorSetLayout(vk_context->device, &layoutInfo, 0, &vk_context->set_layout));
	}
	...
	// Create Pipeline Layout
	{
		...
		VK_CHECK(vkCreatePipelineLayout(vk_context->device, &layoutInfo, 0, &vk_context->pipeline_layout));
	}
	...
	// Sync Objects
	{
		...
		VK_CHECK(vkCreateFence(vk_context->device, &fenceInfo, 0, &vk_context->imgAvailableFence));
	}
	// Create Descriptor Set Layouts
	{
		VkDescriptorSetLayoutBinding binding = {};
	...
	// Staging Buffer
	{
		...
		bufferInfo.size = MB(7);
		VK_CHECK(vkCreateBuffer(vk_context->device, &bufferInfo, 0, &vk_context->staging_buffer.buffer));

		VkMemoryRequirements memRequirements;
		vkGetBufferMemoryRequirements(vk_context->device, vk_context->staging_buffer.buffer, &memRequirements);
		...
		VK_CHECK(vkAllocateMemory(vk_context->device, &allocInfo, 0, &vk_context->staging_buffer.memory));
		VK_CHECK(vkMapMemory(vk_context->device, vk_context->staging_buffer.memory, 0, memRequirements.size, 0, &vk_context->staging_buffer.data));
	...
	// Create Image
	{
		...
		VK_CHECK(vkCreateImage(vk_context->device, &imgInfo, 0, &vk_context->image.image));
		...
		VK_CHECK(vkAllocateMemory(vk_context->device, &allocInfo, 0, &vk_context->image.memory));
		VK_CHECK(vkBindImageMemory(vk_context->device, vk_context->image.image, vk_context->image.memory, 0));

		VkCommandBuffer cmd = {};
		VkCommandBufferAllocateInfo cmd_alloc = cmd_alloc_info(vk_context->command_pool);
		VK_CHECK(vkAllocateCommandBuffers(vk_context->device, &cmd_alloc, &cmd));
		...
		vkCmdCopyBufferToImage(cmd, vk_context->staging_buffer.buffer, vk_context->image.image,
		...
		VK_CHECK(vkCreateFence(vk_context->device, &fenceInfo, 0, &uploadFence));

		VkSubmitInfo submitInfo = submit_info(&cmd);
		VK_CHECK(vkQueueSubmit(vk_context->graphics_queue, 1, &submitInfo, uploadFence));

		VK_CHECK(vkWaitForFences(vk_context->device, 1, &uploadFence, true, UINT64_MAX));
	}
	...
	// Create Image View
	{
		...
		VK_CHECK(vkCreateImageView(vk_context->device, &viewInfo, 0, &vk_context->image.view));
	}
	...
	// Create Sampler
	{
		...
		VK_CHECK(vkCreateSampler(vk_context->device, &samplerInfo, 0, &vk_context->sampler));
	}
	...
	// Create Descriptor Pool
	{
		...
		VK_CHECK(vkCreateDescriptorPool(vk_context->device, &poolInfo, 0, &vk_context->desc_pool));
	}
	...
	// Create Descriptor Set
	{
		...
		VK_CHECK(vkAllocateDescriptorSets(vk_context->device, &allocInfo, &vk_context->desc_set));
	}
	// Update Descriptor Set
	{
		VkDescriptorImageInfo imgInfo = {};
		imgInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		imgInfo.imageView = vk_context->image.view;
		imgInfo.sampler = vk_context->sampler;

		VkWriteDescriptorSet write = {};
		write.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		write.dstSet = vk_context->desc_set;
...
bool vk_render(VkContext *vkcontext)
{
	...
	VK_CHECK(vkAcquireNextImageKHR(vkcontext->device, vkcontext->swapchain, UINT64_MAX, vkcontext->acuire_semaphore, 0, &imgIdx));
	...
	// Rendering Commands
	{
		...
		vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, vkcontext->pipeline_layout,
								0, 1, &vkcontext->desc_set, 0, 0);
	...
	VK_CHECK(vkQueueSubmit(vkcontext->graphics_queue, 1, &submitInfo, vkcontext->imgAvailableFence));
